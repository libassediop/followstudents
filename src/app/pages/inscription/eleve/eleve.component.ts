...
export class EleveComponent implements OnInit {
  ...
  constructor(private zone: NgZone, private changeDetectorRef: ChangeDetectorRef, private route: Router, private professeurService: ProfesseurService, private fb: FormBuilder, private classeService: ClasseService, private serviceInscription: InscriptionreinscriptionService) {
    ...
    this.dateInscription = this.formatDate(new Date());
  }
  ...
  ngOnInit(): void {
    this.formInscription = this.fb.group({
      ...
      telephone: [{ value: '', disabled: false }, Validators.pattern('[0-9]+')],
      ...
      telephoneParent: [{ value: '', disabled: false }, Validators.pattern('[0-9]+')],
  ...
  recuperation($event: Event) {
    ...
    this.classeService.getClasseById(this.formInscription.value.classeId).subscribe(value => {
      ...
      // Vérifier si les montants sont négatifs
      if (montantInscription < 0 || montantMensualite < 0) {
        Swal.fire({
          position: 'center',
          icon: 'error',
          title: 'Erreur : Le montant de réduction doit être inférieur au montant initial.',
          showConfirmButton: false,
          timer: 4500
        });
        return; // Sortir de la fonction si un montant est négatif
      }
